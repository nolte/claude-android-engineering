...
- **File logging and verbosity:** long captures go to a file with rotation (`adb logcat -f <file> -r <kbytes> -n <count>`); verbose tags are toggled at runtime through `Log.isLoggable`-style gating (`adb shell setprop log.tag.<TAG> VERBOSE`); Timber is the de-facto app-side logger (trees planted in debug builds only).

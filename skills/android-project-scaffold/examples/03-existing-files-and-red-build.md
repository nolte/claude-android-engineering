...
3. Verify phase: `./gradlew build` comes back **red** (a leftover kapt application in the pre-existing `app/build.gradle.kts`).
4. The skill reports the full failing task output, diagnoses the kapt residue against the blueprint, and proposes migrating it to KSP (REQ-7 / REQ-9). It applies the fix only at the approval gate, then re-runs the verify sequence.

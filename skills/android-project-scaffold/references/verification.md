...
8. Logging — five sub-checks, because `spec/android/logging/` §G rejects verifying the rule's presence instead of its effect. Report them individually, never step 8 wholesale: only the last needs a device.
   - **§A source** — `grep -rnE --include='*.kt' 'android\.util\.Log|System\.out|\bprintln\(|printStackTrace' app/src` returns hits only under `core/logging/`. Search `app/src`, not `app/src/main`: the scaffold also writes `src/debug/` and `src/release/` sources. Restrict to `*.kt` so the keep file, which names `android.util.Log` on the fallback path, is not itself a hit.
   - **§G rule** — the stripping rule is present where this project keeps it: `app/src/release/keepRules/*.keep` on AGP ≥ 9.3, `app/proguard-rules.pro` below that, or the recorded `-assumenosideeffects` fallback. Check the location the project actually uses, not all three.
   ...
   - **§H gate** — where the §A gate was adopted, it is wired into `task lint` and was seen to fail once on a deliberate `println("x")` placed outside `core/logging/`. Where it was declined, `docs/decisions.md` records the unmet MUST with its reason.
...
| Facade is the only caller of `android.util.Log`; stripping rule present where this AGP generation keeps it; no `int v(`/`int d(` in the release dex; `LogConditional` enabled; the §A gate is wired and was seen to fire, or its absence is a recorded unmet MUST | LOG §A/§G/§H |

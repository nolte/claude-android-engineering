...
Any step that cannot run in the current environment (no Gradle distribution, no Task binary, no device or emulator for steps 6–8, no `apkanalyzer` on the `PATH` for step 8) is named in the final report **with its reason** — an unrunnable check is never treated as green (RR §E). The report lists: step, reason, and what the operator runs to close it.

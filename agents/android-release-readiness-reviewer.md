...
- **Logging:** `Log.v`/`Log.d` (or Timber `DebugTree` planted unconditionally) with no `-assumenosideeffects class android.util.Log { … }` rule and no release-safe tree → Critical; `HttpLoggingInterceptor` at `BODY`/`HEADERS` reachable outside a debug source set or `BuildConfig.DEBUG` guard → Critical; log calls whose arguments carry tokens, credentials, headers, request/response bodies, or personal data → Critical (`security` §A).

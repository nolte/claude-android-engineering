...
- **§F sensitive input:** a hand-built username/password form where Credential Manager applies; a fillable field without a `contentType` semantic; a secret in a plain `TextField` with a visual transformation instead of `SecureTextField` — unless the project records the version deviation `spec/android/user-input-validation/` §B admits, under which that fallback is conformant; autocorrect or suggestions on a secret field; paste blocked on a credential field.

...
- **MUSS [MUST]** die berichtete Startzahl mit einem Macrobenchmark-`StartupTimingMetric`-Lauf erzeugen [R3]; `am start -W` und die Logcat-Zeile `ActivityManager: Displayed` sind nur eine schnelle lokale Kontrolle, messen allein TTID und **DÜRFEN NICHT [MUST NOT]** Grundlage eines berichteten Befunds sein
...
- **DARF NICHT [MUST NOT]** einen Jank-Befund für eine Compose-Oberfläche allein auf `dumpsys gfxinfo framestats` stützen: Die Herstellerdokumentation begrenzt dieses Instrument auf View-Toolkit-Apps [R2]. Es bleibt als grobes lokales Signal und für View-basierte Screens nützlich, die berichtete Zahl kommt aber aus `FrameTimingMetric`
...
- [ ] Startzahlen stammen aus `StartupTimingMetric` und Framezahlen aus `FrameTimingMetric`; `am start -W` und `gfxinfo` erscheinen nur als lokale Kontrollen, und kein Compose-Jank-Befund ruht allein auf `gfxinfo`

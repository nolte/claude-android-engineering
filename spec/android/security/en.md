...
- **MUST** store all private data in internal storage (`MODE_PRIVATE`); **MUST NOT** put sensitive data in external storage, world-readable/writable modes, or cross-app SharedPreferences, and **MUST NOT** log sensitive data (`spec/android/logging/` §C owns what that means for a log line; its §G owns the release-build stripping, which only happens with minification on and the rule present)

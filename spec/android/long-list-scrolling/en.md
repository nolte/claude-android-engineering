...
- Experimental prefetch and scroll indicators: adopt `LazyLayoutCacheWindow` now (its entry points still carry `@ExperimentalFoundationApi` in the current stable line) or wait for the stable API? And which scroll-indicator API is the house choice once one is stable: foundation's `ScrollIndicatorState` (stable, exposed as `ScrollableState.scrollIndicatorState`) is the shipped building block, whereas the `Modifier.scrollIndicator`/`ScrollIndicatorFactory` pair announced in `1.11.0-alpha01` is absent from the `1.11.0` and `1.12.0` API surface, and Material 3's `Modifier.nonInteractiveScrollbar(state = …)` sits in the `1.5.0-alpha` line [R11][R12][R25]. Working default: recorded decision per project, never a generation default; no generated list draws a scrollbar until one of the two ships stable.
...
Sources retrieved 2026-08-11; every version and API-availability statement re-verified 2026-08-12 and again 2026-08-19 (R25 added). Class markers: (P) primary/authoritative vendor documentation and vendor release notes, (S) secondary — vendor-authored engineering articles, maintained tool documentation, third-party API indexes, community-reported behaviour, and published UX research.

**Version statements are dated observations, not pins.** No requirement in this spec is gated on a version number; the two that concern experimental APIs are gated on the stability annotation a reader is required to re-check in the project's own dependency set (§C, §E). Observed 2026-08-19: the latest stable `compose-foundation` is `1.12.0` and the latest stable `compose-material3` is `1.4.0` (both 2026-08-12), and the current Compose BOM `2026.08.00` maps to exactly those two — a BOM can lag its libraries, so the API surface a project actually has follows its BOM, not the newest release [R11][R12][R13]. `LazyLayoutCacheWindow`'s entry points (`rememberLazyListState(cacheWindow = …)`, the state constructors) still carried `@ExperimentalFoundationApi` in that stable line [R14][R24][R25], with a "make `LazyLayoutCacheWindow` stable" entry appearing in the `1.13.0-alpha01` notes [R11]; `ScrollIndicatorState` is stable, the `Modifier.scrollIndicator` modifier announced for `1.11.0-alpha01` is not part of the `1.11.0`/`1.12.0` public API, and the Material 3 scrollbar (`Modifier.nonInteractiveScrollbar`) sat in the `1.5.0-alpha` line [R11][R12][R25]. These figures will age faster than the requirements they illustrate.
...
- [R11] Compose Foundation release notes — `LazyLayoutCacheWindow` availability and stabilisation, `ScrollIndicatorState` (`1.10.0-alpha02`), the `Modifier.scrollIndicator` announcement (`1.11.0-alpha01`), prefetch-scheduler fixes (P): <https://developer.android.com/jetpack/androidx/releases/compose-foundation>
...
- [R25] AndroidX API signature files (source of truth for what a release exposes) — `compose/foundation/foundation/api/1.12.0-beta01.txt` (no `scrollIndicator` modifier; `ScrollIndicatorState` unannotated; cache-window entry points `@ExperimentalFoundationApi`) and `compose/material3/material3/api/current.txt` (`nonInteractiveScrollbar`) (P): <https://android.googlesource.com/platform/frameworks/support/+/androidx-main/compose/foundation/foundation/api/> and <https://android.googlesource.com/platform/frameworks/support/+/androidx-main/compose/material3/material3/api/current.txt>

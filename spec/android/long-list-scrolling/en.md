...
- **MUST NOT** nest two scroll containers of the same direction: a `LazyColumn` inside a `Modifier.verticalScroll` throws `IllegalStateException` [R1]. Headers, footers, banners, and interleaved sections belong **inside** the same lazy container via its `item`/`items` DSL. A cross-direction nest (`LazyRow` inside a `LazyColumn` item) is legal; a same-direction nest is legal only when the inner container has a fixed size [R1], which **SHOULD** be treated as a smell — two scroll surfaces then compete for one gesture

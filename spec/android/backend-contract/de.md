...
- **MUSS [MUST]** die *eigenen* transparenten Wiederholungen des Transports berücksichtigen, die die Regel oben nicht sieht: OkHttps `retryOnConnectionFailure` steht per Default auf `true` und sendet einen Request — samt Body — stillschweigend erneut bei einer abgestandenen gepoolten Verbindung, einer unerreichbaren Adresse eines Multi-homed-Hosts, einem `408` oder einem `401`/`407`, das der `Authenticator` beantwortet [R11]. Für einen nicht idempotenten Schreibvorgang ohne beachteten Idempotenzschlüssel ist das ein verkleideter Doppel-Write; der Client deaktiviert deshalb entweder `retryOnConnectionFailure` (am gemeinsamen Client oder an einem eigenen Write-Client) oder markiert solche Request-Bodies als one-shot (`RequestBody.isOneShot() == true`), was OkHttp am erneuten Senden hindert [R11]. Die Wahl wird festgehalten; ein nackter Default-Client hinter nicht idempotenten Schreibvorgängen ist nicht konform
...
- [ ] Timeouts sind explizit; automatische Wiederholungen gibt es nur für idempotente Requests oder geschlüsselte Schreibvorgänge, mit Jitter, Höchstzahl an Versuchen und Frist; OkHttps transparente Wiederholung ist deaktiviert oder die Bodies sind one-shot für nicht idempotente Schreibvorgänge; die Credential-Erneuerung läuft im Single-Flight

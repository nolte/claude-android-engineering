...
Provenienz: Schreibtischrecherche (August 2026) über die Dokumentation des OpenAPI-Generator-Kotlin-Clients, RFC 9457 (Problem Details for HTTP APIs), den (inzwischen abgelaufenen) IETF-HTTPAPI-Entwurf zum `Idempotency-Key`-Header, RFC 9110 für die Status-Semantik, die OkHttp-Client-Dokumentation, die Android-Leitfäden zu Architektur und Offline-First sowie die konsumierende App dieses Portfolios (`nolte/kamerplanter-android`, ein OpenAPI-generierter Retrofit-Client in `core/network`).
...
- **SOLLTE [SHOULD]** den Kotlin-Generator so konfigurieren: `library` = `jvm-retrofit2` mit `useCoroutines` oder `jvm-ktor`, wo bereits ein Ktor-Stack existiert; `serializationLibrary` = `kotlinx_serialization`; `dateLibrary` = `java8` (reine JVM-Apps). Das ist eine **Portfolio-Vorgabe**, keine Herstellerempfehlung — der Generator unterstützt mehrere Kombinationen, und [R1] wird nur für die Optionsnamen und ihre zulässigen Werte zitiert. Eine andere Kombination ist erlaubt, **MUSS [MUST]** aber mit Begründung festgehalten werden. Drei Generator-Fakten, die ein Skill nicht erst mitten im Lauf entdecken darf: `useCoroutines` ist ein Schalter nur für `jvm-retrofit2` [R1]; `jvm-ktor` (und `multiplatform`) generieren gegen Ktor 1.6.7 [R1], ein Projekt auf Ktor 3 hält also fest, warum es diese Library dennoch nutzt (oder welche Template-Überschreibung es anwendet), statt einen aktuellen Client vorauszusetzen; und `enumUnknownDefaultCase=true` gibt jedem generierten Enum das Rückfallmitglied (`unknown_default_open_api`), das §D verlangt, sodass der Rückfall generiert und nicht von Hand nachgepatcht wird [R9]
...
- **MUSS [MUST]** die folgende Status-Zuordnung für die Codes anwenden, die die acht Fälle implizit lassen, damit ein Skill eine Tabelle anwendet statt je Feature zu entscheiden [R10]:
  - `304 Not Modified` auf einen bedingten Request (§C) ist **Erfolg ohne Body**: Die Replica ist als frisch bestätigt, ihre Frische-Metadaten werden aktualisiert, nichts wird neu geschrieben
  - `404`/`410` auf eine Ressource, die die Replica bereits hält, ist eine **fachliche Ablehnung** des *Zustands* der Ressource — das Element wurde serverseitig gelöscht oder zurückgezogen. Der Client entfernt oder tombstoned die Replica-Zeile gemäß `spec/android/app-architecture/` §D und zeigt einen „nicht mehr verfügbar"-Zustand, kein Fehlerbanner. Ein `404` für einen Endpunkt, den der Contract dokumentiert (nicht für ein Element), ist eine **Contract-Abweichung**
  - `412 Precondition Failed` auf `If-Match`/`If-Unmodified-Since` ist eine **fachliche Ablehnung** in ihrer Konfliktform: Die Ressource hat sich unter dem Client geändert, der neu lädt und neu präsentiert; er **DARF NICHT [MUST NOT]** mit dem veralteten Validator wiederholen
  - eine Antwort, die der Contract als „App-Version nicht unterstützt" definiert — wie auch immer kodiert (manche Backends borgen `426 Upgrade Required`, dessen Bedeutung nach RFC 9110 ein *Protokoll*-Upgrade ist; ein Problem-`type` ist der saubere Träger) — ist eine **fachliche Ablehnung** mit genau einer Wiederherstellungsaktion, *App aktualisieren*: Der Screen blockiert darauf und wiederholt nie automatisch
  - `401` ist nicht-authentifiziert, `403` nicht-autorisiert, `408` ein Timeout, `429` ein Serverfehler gemäß dem Rate-Limiting-Grenzfall oben, und jeder andere hier nicht zugeordnete `4xx` (`400`, `405`, `409`, `413`, `415`, `422`, …) ist eine **fachliche Ablehnung**; jeder `5xx` ist ein **Serverfehler**. Ein `4xx`, den der Contract mit einer eigenen Bedeutung dokumentiert, die diese Tabelle nicht ausdrücken kann, ist ein §E-Auslöser, keine lokale Vermutung
...
- **MUSS [MUST]** automatisch nur wiederholen, wo der Request idempotent ist — `GET`, `HEAD`, `PUT`, `DELETE` gemäß Contract — mit gedeckeltem exponentiellem Backoff **plus Jitter** (ein zufälliger Anteil der gedeckelten Verzögerung, nie ein fester Zeitplan), einer festgelegten Höchstzahl an Versuchen und einer Gesamtfrist, nach der der Aufruf als gescheitert gemeldet wird — alle drei mit dem Feature festgehalten und nie einem Library-Default überlassen [R4]
...
- **MUSS [MUST]** die *eigenen* transparenten Wiederholungen des Transports berücksichtigen, die die Regel oben nicht sieht: OkHttp sendet einen Request — samt Body — stillschweigend erneut bei einer abgestandenen gepoolten Verbindung, einer unerreichbaren Adresse eines Multi-homed-Hosts, einem `408`, einem `401`/`407`, das der `Authenticator` beantwortet, einem `503` mit `Retry-After: 0` und einem `421` [R11]. Für einen nicht idempotenten Schreibvorgang ohne beachteten Idempotenzschlüssel ist das ein verkleideter Doppel-Write. `retryOnConnectionFailure` (Default `true`) steuert nur das Erholen von Verbindungsfehlern und das `408`-Follow-up; die Follow-ups zu `401`/`407`, `503` und `421` senden den Body unabhängig von diesem Flag erneut [R11]. Nur ein One-shot-Body (`RequestBody.isOneShot() == true`) unterbindet jedes automatische erneute Senden, deshalb markiert der Client die Bodies nicht idempotenter Schreibvorgänge als one-shot (oder installiert einen Interceptor, der ein erneutes Senden eines solchen Requests ablehnt); `retryOnConnectionFailure` am gemeinsamen oder einem eigenen Write-Client zu deaktivieren ist eine zusätzliche Härtung, keine hinreichende [R11]. Die Wahl wird festgehalten; ein nackter Default-Client hinter nicht idempotenten Schreibvorgängen ist nicht konform
- **SOLLTE [SHOULD]**, sofern das Backend es unterstützt, `Idempotency-Key` als clientseitig erzeugte UUID senden, die über Wiederholungen *desselben* logischen Schreibvorgangs stabil bleibt und für einen neuen wechselt; der Schlüssel wird mit einem eingereihten Schreibvorgang persistiert, damit er den Prozesstod überlebt [R3][R8]. Der Header ist nur durch einen IETF-Entwurf spezifiziert, der inzwischen **abgelaufen** ist (`draft-ietf-httpapi-idempotency-key-header-07`, zuletzt überarbeitet am 15.10.2025, abgelaufen am 18.04.2026, zum Zeitpunkt des Schreibens ohne Nachfolger) — er ist De-facto-Herstellerpraxis, kein Standard [R3]; deshalb bleibt dies ein **SOLLTE**, das daran gebunden ist, dass das Backend ihn tatsächlich beachtet, Header-Name und Semantik werden dem Contract des Backends entnommen, der Mechanismus ist gleichwohl etablierte, ausgelieferte Praxis bei mehreren Zahlungsanbietern [R8], und wo ein Backend keinen anbietet, greift stattdessen das Verbot aus §C, einen nicht idempotenten Schreibvorgang automatisch zu wiederholen
...
- **MUSS [MUST]** jedem Enum ein Rückfallmitglied geben (generiert über `enumUnknownDefaultCase=true`, §A) und **DARF NICHT [MUST NOT]** einen unbekannten Enum-Wert als fatalen Fehler behandeln; die UI zeigt eine neutrale Darstellung, und der Fall wird als Contract-Beobachtung geloggt
...
- [ ] Timeouts sind explizit; automatische Wiederholungen gibt es nur für idempotente Requests oder geschlüsselte Schreibvorgänge, mit Jitter, Höchstzahl an Versuchen und Frist; die Bodies nicht idempotenter Schreibvorgänge sind one-shot (oder ein Interceptor lehnt ihr erneutes Senden ab), `retryOnConnectionFailure=false` höchstens als Ergänzung; die Credential-Erneuerung läuft im Single-Flight
...
- [ ] Die Status-Tabelle aus §B ist angewandt: `304` aktualisiert nur die Frische, `404`/`410` auf eine gecachte Ressource bereinigt die Replica und zeigt einen „nicht mehr verfügbar"-Zustand, `412` lädt neu, ohne den veralteten Validator zu wiederholen, eine Antwort „Version nicht unterstützt" blockiert mit einer Aktualisierungsaktion, und nicht zugeordnete `4xx` sind fachliche Ablehnungen
...
- [R3] IETF HTTPAPI WG — The Idempotency-Key HTTP Header Field (Internet-Draft, **abgelaufen** — `-07` vom 15.10.2025, abgelaufen am 18.04.2026; clientseitige Schlüsselerzeugung, serverseitige Behandlung von Duplikaten/Nebenläufigkeit, 409/422-Semantik): <https://datatracker.ietf.org/doc/draft-ietf-httpapi-idempotency-key-header/>
...
- [R9] OpenAPI Generator — Option `enumUnknownDefaultCase` (fügt jedem Enum `unknown_default_open_api` hinzu; vom kotlin-client-Template `enum_class` beachtet): <https://github.com/OpenAPITools/openapi-generator/blob/master/modules/openapi-generator/src/main/resources/kotlin-client/enum_class.mustache>
...
- [R11] OkHttp — `OkHttpClient.Builder.retryOnConnectionFailure` (Default true; stilles Erholen von unerreichbaren Adressen, abgestandenen gepoolten Verbindungen, Proxies) und `RequestBody.isOneShot` (Fälle des erneuten Sendens: abgestandene Verbindung, 408, 401/407, 503 mit `Retry-After: 0`, 421): <https://github.com/square/okhttp/blob/master/okhttp/src/commonJvmAndroid/kotlin/okhttp3/OkHttpClient.kt>

...
- Transportsicherheit, Entwurf des Authentifizierungsverfahrens und Credential-Speicherung — `spec/android/security/` §C/§G
...
### B. Die geschlossene Fehlermenge

- **MUSS [MUST]** jedes Ergebnis eines Aufrufs auf genau einen von acht Fällen abbilden und **MUSS [MUST]** alle acht je Feature behandeln (die Fakes aus `spec/android/app-architecture/` §G existieren, um das nachzuweisen):
  1. **Erfolg**
...
- **SOLLTE [SHOULD]** nach Möglichkeit einen Request je Screen-Zustand halten; ein Screen, der drei oder mehr Aufrufe für seinen ersten Frame braucht, ist ein §E-Auslöser und keine clientseitige Orchestrierungsübung
...
- **MUSS [MUST]** diese Abschnitte in dieser Reihenfolge enthalten, damit die Backend-Seite ohne Rückfrage umsetzen kann:
  ...
  5. **Nicht-funktionale Bedarfe** — Latenzbudget des konsumierenden Screens, erwartete Seitengröße und Sortierung, Auth-Scope, Idempotenzbedarf, Cachebarkeit und Validatoren, Datenvolumen
...
- **MUSS [MUST]** ausschließlich synthetische Beispiele verwenden; keine personenbezogenen Produktivdaten, keine Credentials, keine echten Nutzerkennungen (`spec/android/security/` §E)

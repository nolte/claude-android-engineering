...
- **MUST** account for the transport's *own* transparent retries, which the rule above does not see: OkHttp silently re-sends a request — body included — on a stale pooled connection, an unreachable address of a multi-homed host, a `408`, a `401`/`407` the `Authenticator` satisfies, a `503` with `Retry-After: 0`, and a `421` [R11]. For a non-idempotent write without an honoured idempotency key that is a duplicate write in disguise. `retryOnConnectionFailure` (default `true`) gates only the connection-failure recovery and the `408` follow-up; the `401`/`407`, `503`, and `421` follow-ups re-transmit the body regardless of that flag [R11]. Only a one-shot body (`RequestBody.isOneShot() == true`) stops every automatic re-send, so the client marks the bodies of non-idempotent writes one-shot (or installs an interceptor that rejects a re-send of such a request); disabling `retryOnConnectionFailure` on the shared or a dedicated write client is an additional hardening, not a sufficient one [R11]. The choice is recorded; a bare default client behind non-idempotent writes is non-conformant
...
- [ ] Timeouts are explicit; automatic retries exist only for idempotent requests or keyed writes, with jitter, a maximum attempt count, and a deadline; the bodies of non-idempotent writes are one-shot (or an interceptor rejects their re-send), with `retryOnConnectionFailure=false` at most as an addition; credential refresh is single-flight
